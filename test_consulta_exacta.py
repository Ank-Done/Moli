#!/usr/bin/env python3
"""
Probar EXACTAMENTE tu consulta para obtener los datos que mostraste
"""
...
def test_consulta_exacta():
    db = DatabaseConfig()
    
    # TU CONSULTA EXACTA que proporcionaste
    query = """
    SELECT
    ...
    print("=== EJECUTANDO TU CONSULTA EXACTA ===")
    df = db.execute_query(query)
    print(f"Registros obtenidos: {len(df)}")
    
    print("\n=== VERIFICANDO COINCIDENCIA CON TUS DATOS ===")
    if len(df) > 0:
        print("Primeros 10 registros:")
        for _, row in df.head(10).iterrows():
            print(f"{int(row['Anio'])}    {int(row['Mes'])}    {row['KilosTotales']:.12g}    {row['ToneladasTotales']:.12g}")
    
    # Verificar registro específico que proporcionaste
    primer_registro = df.iloc[0] if len(df) > 0 else None
    if primer_registro is not None:
        esperado_kilos = 18901903.94799999
        esperado_toneladas = 18901.903947999992
        actual_kilos = float(primer_registro['KilosTotales'])
        actual_toneladas = float(primer_registro['ToneladasTotales'])
        
        print(f"\n=== COMPARACIÓN PRIMER REGISTRO ===")
        print(f"Esperado: 2018-1 -> {esperado_kilos} kg, {esperado_toneladas} ton")
        print(f"Obtenido: {int(primer_registro['Anio'])}-{int(primer_registro['Mes'])} -> {actual_kilos} kg, {actual_toneladas} ton")
        print(f"¿Coincide?: {'✅ SÍ' if abs(actual_kilos - esperado_kilos) < 1 else '❌ NO'}")

if __name__ == "__main__":
    test_consulta_exacta()